...
int req_dirwalker(endpoint_t fs_e,char *fullpath,ino_t link_parent)
{
  int r;
  message m;
  cp_grant_id_t grant_id;
  const size_t len = strlen(fullpath) + 1;
  

  grant_id = cpf_grant_direct(fs_e, (vir_bytes)fullpath, len, CPF_READ);
  if(grant_id == -1)
	printf("\n gran failed and returned -1 \n");

  /* Fill in request message */
  m.m_type = REQ_DIRWALKER;
  m.REQ_GRANT = grant_id;
  m.REQ_PATH_LEN = len;
  m.REQ_DIR_INO = link_parent;
  m.REQ_ROOT_INO =link_parent;
